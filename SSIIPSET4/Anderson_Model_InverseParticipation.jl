...
function InverseParticipation(L::Int64,EigSol::Array{Float64,2},q::Number)
    ...
    for i in 1:L#Indexes states
        ...
        for j in 1:L #Indexes positions
            INVPR = INVPR + (norm(Proj(L,j)*States[j]))^(2q)
        end
...
plot(Xaxis,Yaxis,title = "Inverse Participation", label =  nothing, #=seriestype = :scatter,=# xlabel = "q", ylabel = "IPR")
